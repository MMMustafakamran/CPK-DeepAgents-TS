 ...
 * is set, and then the memory routes the hook calls are the platform's, not a
 * mock. This repo has no key (locally or in CI), so it runs in SSE mode — and
 * there the hook never gets as far as a request: `CopilotKitCore` hands the
 * memory store a context only when `/info` reports an Intelligence `wsUrl`, so
 * the store sits with no context, `isAvailable` keeps its initial `true`,
 * the list is empty, `realtimeStatus` stays `connecting`, and `addMemory`
 * rejects in the browser with "Runtime URL is not configured". The page says an
 * unentitled deployment looks like `isAvailable: false`; this looks like an
 * empty memory list.
 *
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="min-h-0 flex-1">
        {/* This runtime registers no `default` agent; the harness names its own. */}
