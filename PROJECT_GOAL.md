...
QA on the CopilotKit React DeepAgents docs, read in their **TypeScript** tabs
(<https://docs.copilotkit.ai/deepagents>). The job is **finding bugs and
ambiguity in those doc pages**. The Python tabs are the sibling repo's job
(`DeepAgentspy-react`); where the two languages diverge, that divergence is
itself a finding. The deliverable is `DOCUMENTED_REPORT.md`, built by `ci/build-report.mjs`, plus one
...
- **Divergence from the Python build** of the same guide; nothing compares them.
  Four places where the languages genuinely differ are recorded in README §9.
- **Pages with no `/demo-chat` route** — `shared-state/state-inputs-outputs` and
  `shared-state/workflow-execution` are reference-only here, so they are in no
  recording and no drift clip. Check them by hand.
