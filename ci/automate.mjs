...
async function main() {
  ...
  try {
    ...
    // 3. Dependencies
    if (!skipInstall) {
      // The backend is Node here, not Python: `langgraph.json` declares
      // `"package_manager": "npm"` and the CLI expects the graphs' dependencies
      // already installed in backend/node_modules.
      installNodeDeps(BACKEND_DIR, 'Installing Backend Dependencies');
    ...
    if (busy.backend) {
      ...
      const backend = spawnServer(
        `npx @langchain/langgraph-cli dev --port ${BACKEND_PORT} --no-browser`,
        BACKEND_DIR,
        'backend.log',
      );
