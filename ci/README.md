...
├── run-name.mjs          names the run's artifacts (DeepAgentsjs-React-28Aug2026-0527UTC)
...
3. **Dependencies** — `npm install` for the backend, the frontend
   and recorder.
...
`DAJS-react-06-InterruptBased.webm`, and step 7 applies it. To narrate a page,
...
|---|---|---|---|
| Agent | 8124 | `npx @langchain/langgraph-cli dev --port 8124 --no-browser` | `/ok` |
...
`npm install` picks the newest versions the ranges in each `package.json`
already allow. `@copilotkit/*` is a caret
...
```
DeepAgentsjs-React-28Aug2026-0527UTC             ← consolidated, all clips + report
DeepAgentsjs-React-28Aug2026-0527UTC-shard-1     ← one worker's output
```
