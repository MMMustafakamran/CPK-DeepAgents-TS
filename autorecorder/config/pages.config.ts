  ...
  // no OPENAI_API_KEY (a local run) the handlers note the silence and go on.
  {
    ...
    knownIssue: {
      ...
      note: [
        ...
        '',
        'top pane = steps 2+3 as published, no agentId anywhere',
        "so useAgent() asks for agent 'default'. this runtime only has sample_agent etc",
        "-> Agent 'default' not found after runtime sync (the overlay).",
        'whole route dies without the error boundary',
        '',
        'bottom pane = same code + agentId sample_agent',
        'card renders, payload row says only user went out. the idea works,',
  ...
  },
  {
    ...
    knownIssue: {
      ...
      note: [
        ...
        'with /v2 on our runtime (no intelligence key): isAvailable true, 0 memories,',
        'realtime stuck on connecting. no /memories request ever leaves the browser',
        'page says unavailable memory reads isAvailable false. it does not',
        '',
        'memory.access runtime (the option the page never mentions) = the 503s, no key',
      ].join('\n'),
    },
  },
  {
    ...
    knownIssue: {
      ...
      note: [
        ...
        'no CPK_INTELLIGENCE_API_KEY -> constructor throws at module load -> 500s',
        '(that is the terminal). runtime row says error, prompt sits in the box,',
        'neither tab answers',
        '',
