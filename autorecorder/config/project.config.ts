...
export const PROJECT: ProjectConfig = {
  ...
  frameworkLabel: 'Deep Agents (TypeScript)',
  videoPrefix: 'DAJS-react',

  docBaseUrl: 'https://docs.copilotkit.ai/deepagents',

  frontendUrl: process.env.FRONTEND_URL || 'http://localhost:3000',

  // 8124, not the Quickstart's 8123. This backend is not a FastAPI app -- it is
  // the LangGraph JS dev server, which serves every graph in
  // `backend/langgraph.json` and answers `/ok` rather than `/health`. The port
  // differs from the doc's on purpose: the Python sibling repo publishes
  // identical graph ids on 8123, so on a shared port whichever server bound
  // first would silently answer for the other language. Change the port here
  // and `LANGGRAPH_DEPLOYMENT_URL` in `frontend/.env.local` has to change with
  // it, or the runtime route keeps forwarding runs to the old one.
